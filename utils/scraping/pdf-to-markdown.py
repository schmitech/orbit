...
A command-line tool for converting PDF documents to Markdown format
using the docling library. This tool provides flexible input/output options and
comprehensive error handling.

Features:
- Convert single PDF documents to Markdown
...
Usage Examples:
    ...
    python pdf-to-markdown.py input.pdf output.md
    
    # Convert a file with automatic output naming
    python pdf-to-markdown.py document.pdf
    
    # Convert with verbose output
    python pdf-to-markdown.py -v input.pdf output.md
    
    # Exclude table of contents from output
    python pdf-to-markdown.py --exclude-toc document.pdf
    
    # Exclude page numbers from output
    python pdf-to-markdown.py --exclude-page-numbers document.pdf
    
    # Exclude both ToC and page numbers
    python pdf-to-markdown.py --exclude-toc --exclude-page-numbers document.pdf
    
    # Combine filtering with verbose output
    python pdf-to-markdown.py --exclude-toc --exclude-page-numbers -v document.pdf clean.md
    
    # Show help
    python pdf-to-markdown.py -h
...
def is_page_number(text):
    ...
    # Check for common page number formats in PDFs
    if re.match(r'^-\s*\d+\s*-$', text):  # - 5 -
        return True
    
    if re.match(r'^page\s+\d+$', text, re.IGNORECASE):  # Page 5
        return True
...
def enhance_content_for_qa(content):
    ...
    # Fix hyphenated words broken across lines (very common in PDFs)
    content = re.sub(r'(\w)-\n(\w)', r'\1\2', content)
...
def convert_pdf_to_markdown(input_file, output_file, verbose=False, exclude_toc=False, exclude_page_numbers=False, enhance_for_qa=True):
    """
    Convert a PDF document to Markdown format.
    
    Args:
        input_file (str): Path to the input PDF document
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s document.pdf                    # Convert with auto-generated output name
  %(prog)s input.pdf output.md             # Convert with custom output name
  %(prog)s -v document.pdf                 # Verbose output
  %(prog)s --exclude-toc document.pdf      # Exclude table of contents
  %(prog)s --exclude-page-numbers doc.pdf  # Exclude page numbers
  %(prog)s --exclude-toc --exclude-page-numbers doc.pdf # Exclude both
  %(prog)s --help                          # Show this help message
    ...
    parser.add_argument(
        'input_file',
        help='Path to the input PDF document (.pdf)'
    )
    ...
    parser.add_argument(
        '--version',
        action='version',
        version='PDF to Markdown Converter v1.0'
    )
