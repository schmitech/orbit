...
from generate_import_config import (
    match_columns,
    generate_yaml as _generate_import_yaml,
    read_csv_headers,
)
...
def generate_import_config(
    ...
    """Generate import_config.yaml with real column mappings.

    Matches schema *columns* against CSV headers using slug-based
    fuzzy matching from generate_import_config.py.
    """
    ...
    mapping, unmatched_schema, unmatched_csv = match_columns(columns, csv_headers)
    return _generate_import_yaml(mapping, unmatched_schema, unmatched_csv)
  ...
  python generate_schema.py data.csv --type-override amount=DECIMAL(12,2)
    ...
    if args.dry_run:
        ...
        if args.config:
            ...
            print("=" * 60)
            config_content = generate_import_config(
                columns,
                str(csv_path),
                delimiter=delimiter,
                quote=quote,
