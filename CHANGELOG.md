...
- **Conversation-Aware Language Detection**: Short or ambiguous follow-ups such as "OK" now keep the language of the conversation instead of becoming unknown. Each message's detected language is saved with the conversation, and the next message uses it. Only confident detections count, the most recent messages weigh most, and a message clearly in a new language is followed right away. Config: `chat_history_prior_weight` is removed; `prior_min_confidence` and `stickiness_ttl_seconds` are added. See Phase 2 of `docs/roadmap/language-detection-accuracy.md`.
