...
### CLI & Tools
- **`orbit status` Redesign**: `status` now reports health rather than just process liveness. A single renderer (replacing the two divergent ones) merges `/metrics/json`, `/health/ready`, and `/health/system` with process facts to show adapter readiness, request and error rates, p50/p95/p99 latency, and per-adapter circuit-breaker state, with a `healthy`/`degraded`/`unhealthy` verdict derived from the thresholds the server reports for itself. Unavailable sections -- monitoring disabled, or no admin credentials -- are named rather than silently omitted. Adds `--detailed`, honors the existing global `--output json` (previously ignored by `status`), and makes `--watch` a live render instead of clearing the screen.
