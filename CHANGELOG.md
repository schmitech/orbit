...
- **Settings Tab Search & Collapsible Tree**: The Settings tab's sidebar now supports search-as-you-jump — typing filters sections by name and by content (fetched and cached on first search), surfacing the first matching line under each result and jumping straight to it in the editor on click. Section groups collapse by default and can be toggled individually or all at once via a new "Expand all"/"Collapse all" control, reducing sidebar clutter as more settings sections are added.
- **API Key Panel Usability**: Clarified that an empty user allowlist leaves a key unrestricted, added a clear-selection action, and made selected users consistently blue across platforms. Refined the editor layout with compact controls, bottom-positioned save/cancel actions, inline key testing and renaming, and a compact quota loader. Cancelling an edit now restores the saved Notes preview instead of leaving unsaved text visible. Admin module imports are versioned with the panel entrypoint so refreshed pages receive updated tab code.
