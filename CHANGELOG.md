...
- **MCP HTTP Diagnostics for Failed Discovery**: The MCP Streamable HTTP SDK collapses every non-2xx response into the same generic `"Server returned an error response"` error, discarding the real status code and body -- so a 401 (bad credential) and a 500 (server bug) were indistinguishable from ORBIT's logs, and admin-panel pings gave no way to troubleshoot which. `MCPClientManager` now attaches httpx request/response event hooks to each HTTP-transport connection: the outgoing request (method, URL, header *names* only -- never values, so credentials are never logged) at DEBUG, and any non-2xx response (status, reason, a bounded body snippet) at WARNING. The body read is capped at 500 bytes via `aiter_bytes(chunk_size=500)` so a large or non-terminating error body can't stall discovery or be buffered in full. `_discover_server`'s failure log now also names the exception type and points at these DEBUG logs. No behavior change to discovery outcomes -- purely additive diagnostics. Added dedicated coverage in `test_mcp_client_service.py` for header redaction, the bounded read (including a never-ending body), and end-to-end hook wiring through `_create_connection`.
