...
- **Language Detection Overhaul**: Detection now distinguishes languages that share scripts, abstains on messages without language evidence, and uses trusted conversation history for ambiguous follow-ups. Confidence is calibrated from full backend candidate distributions; prompt and retrieval language behavior now require a sufficiently confident, current-message decision. Per-backend-set calibration also covers partial deployments and timeouts. Mixed-language messages are now reported from detected spans (with offsets and secondary languages, as metadata only) instead of backend disagreement; 6 of 8 held-out mixed prompts are caught, with 2 of 158 single-language prompts falsely flagged. On the held-out benchmark, selective accuracy rose from 92.2% to 94.2% and high-confidence errors fell from 5 to 0. `fallback_language` is now `ambiguous_response_language` (the old key remains supported); see `docs/roadmap/language-detection-accuracy.md` for configuration changes and benchmark details.

### Technical Improvements
- **Language Detection Benchmark**: Added a versioned 334-prompt, 55-language benchmark with frozen tune/held-out splits, regression gates, checked-in baselines, and reproducible calibration fitting in `server/tests/language_eval/`.
...
- **MCP Client Service Cleanup**: Unified connection initialization, protocol compatibility lookups, secret masking, and circuit-breaker checks. Tool discovery now uses concurrency-safe flattened caches and schema indexes, while unused curl-reproduction logging was removed. Runtime behavior remains unchanged.
- **Pipeline Streaming Structured-Event Contract**: Documented the structured-event refactor needed to remove parse-and-reserialize work from the OpenAI-compatible and A2A streaming boundaries. The scoping plan identifies the existing raw-stream API as the canonicalization point and records the contract, scope, compatibility, testing, and consumer-audit decisions required before implementation. No code change.
