...
### Core System Updates
- **Per-Turn Prompt Token Reduction**: Chart-formatting instructions are no longer sent unconditionally on every turn — a new `supports_charts` adapter capability gates them off entirely for non-analytics adapters, and chart-capable adapters get a ~120-token hint instead of the ~1,200-token full spec unless the turn (or recent history) actually looks chart-related. The system message is also now built as a stable prefix (system prompt, chart instruction, persona footer) followed by a volatile tail (language/time instructions, RAG context), so provider-side prompt caching can hit the unchanging prefix instead of being busted every turn by the second-granularity clock instruction. Anthropic now requests an explicit `cache_control` breakpoint on that prefix; OpenAI and Gemini benefit passively from the same stable ordering. See `docs/roadmap/token-optimization-plan.md` for phased rollout status — MCP tool-schema filtering is still tracked there as not yet started.
- **Cache-Token-Aware Cost Reporting**: Usage tracking and pricing now account for provider-side cache hits instead of folding them into full-price prompt tokens. `PricingService` gained an optional `cached_input_per_1m` discount tier per model; DeepSeek (`prompt_cache_hit_tokens`) and xAI (`prompt_tokens_details.cached_tokens`, including the Responses API's `input_tokens_details.cached_tokens` shape used for `web_search=True` requests) now extract and report their cache-hit token counts. Also fixed Anthropic's own usage accounting, which previously undercounted `prompt_tokens` by excluding `cache_read_input_tokens`/`cache_creation_input_tokens` from the total whenever a cache breakpoint hit. Configured discount rates for Anthropic and DeepSeek in `config/pricing.yaml`; providers with no confirmed caching mechanism (Mistral, Cohere) were audited and left unchanged.
...
- **Adapter Test Query Panel**: The Adapters admin panel gained a "Test Query" action for intent and composite retriever adapters, gated by a new `supports_test_query` capability flag. Expands an inline panel to run a natural-language query against the adapter's templates without the full LLM pipeline, rendering template candidates with scores, reranking boosts, the selected template, parameter extraction trace, rendered query, and execution results — surfacing the existing `template_diagnostics` engine that previously had no UI.
- **Per-Example Template Indexing for the HTTP Adapter Family**: `intent_http_base.py` (MongoDB, Elasticsearch, GraphQL, and generic HTTP intent retrievers) now indexes one vector per `nl_example` instead of one blended vector per template, plus the nl_example exact/Jaccard-match rescue path — matching the precision `intent_sql_base.py` already had. Also unified `confidence_threshold` resolution in both bases so the domain adapter's result-filter threshold and the retriever's match-gate threshold read the same resolved value instead of diverging on default (previously `0.75` vs `0.1` for the same unset config key), logged at `INFO` on init.
- **SQL Query Safety Guard**: Added `server/retrievers/base/query_guard.py`, using the new `sqlglot` dependency to reject rendered SQL that isn't a single, read-only, row-capped query before it reaches the database — the enforcement point that makes ORBIT's "no free-form text-to-SQL" claim about intent templates verifiable rather than aspirational. Wired into `intent_sql_base.py::_execute_template` right after SQL rendering and before execution, gated by new adapter-config knobs `query_guard_enabled` (default `true`) and `query_guard_max_rows` (default `1000`); a rejection falls through the existing "try next template" retry path instead of crashing the request. The dialect is resolved from each retriever's real SQL datasource (`postgres`/`mysql`/`mariadb`/`mssql`/`sqlite`/`duckdb`/`oracle`/`athena`) so vendor-specific placeholder syntax parses correctly.

### Bug Fixes & Technical Improvements
- **Template Reranker Vocabulary Mutation**: Fixed `TemplateReranker._calculate_action_boost` appending the matched action onto the domain config's shared `vocabulary.action_verbs[action]` list in place rather than a copy, so the list grew by one entry on every call and inflated action-verb boosts more with each repeated query against the same retriever instance.
- **Stale Validation-Tooling Docs**: Removed `docs/intent-sql-rag-system.md` references to `validate_rag_results.py` and `sql_validation_templates.py`, neither of which exists in the repository, and pointed the Validation System section at the new intent-eval harness instead.
- **Template Search Limit Starvation**: Fixed `_find_best_templates` in both `intent_sql_base.py` and `intent_http_base.py` using a fixed `max_templates * 3` vector-search limit before deduplicating per-example hits down to base templates. A template with many similar `nl_examples` could occupy every slot in that fixed window, pushing other genuinely relevant templates out of the candidate list entirely. The search now widens (doubling, capped at `max_templates * 20`) until deduplication yields `max_templates` distinct templates or the store has nothing left to return.
- **HTTP-Family Templates Executed With Missing Required Parameters**: `intent_http_base.py`'s `get_relevant_context` had no check for required parameters still missing after extraction, unlike the SQL base — so it fell through to a live HTTP/GraphQL call with an unfilled placeholder, which the remote API correctly rejected (e.g. `Unsubstituted template variables`, `Variable "$id" ... was not provided`). Added the same missing-required-parameter skip the SQL base already has, so the next candidate template is tried instead of firing an avoidable network call.
- **Query Guard Dialect Resolved From the Wrong Config Field**: `resolve_dialect()` was called with `self.store_name`, which identifies the template *vector store* (e.g. `chroma`), not the SQL backend — every real deployment (postgres/mysql/sqlite/...) was parsed with sqlglot's generic dialect instead of the correct one. Fixed to resolve from `self._get_datasource_name()`, the method every concrete intent SQL retriever already implements.
- **Query Guard Let TRUNCATE Through**: `assert_read_only`'s deny-list didn't cover `TruncateTable` (nor `Copy`, `Execute`, or sqlglot's generic `Command` fallback for unmodeled syntax), so a rendered `TRUNCATE` passed the guard. Now also requires the top-level statement to be a query (`SELECT`/`UNION`/`INTERSECT`/`EXCEPT`), rejecting every non-query statement type in one check instead of extending the deny-list statement-by-statement.
- **Admin Panel Form UX Pass**: Fixed a CSS specificity bug in the Adapters creation form where checkboxes rendered stacked above their labels instead of beside them, and added missing/inconsistent input guardrails across several tabs — character or entry counters on previously uncapped fields (adapter list/skill fields, the API Keys "pre-authorize emails" input), a bounded field hint reword that no longer implies auto-generation, a consistent icon empty-state for an empty adapter list, and a numbered/ellipsis pager for the Audit ledger (previously showed only the current page number even with more pages available).
...
### Core System Updates
- **User Blacklisting**: Administrators can now deny authentication by identity pattern, to cut off abusive users when user authorization is enforced. A rule matches `email`, `user_id`, or `username` with shell-style wildcards (`*@spam-domain.com`, `entra:abc*`), case-insensitively. Unlike deactivating a user, which needs an existing row in the `users` table, a rule also blocks external (Entra/Auth0) users who have never signed in — `_find_or_create_external_user` refuses to JIT-provision a blacklisted identity, so no account is created at all. Enforcement lives in `AuthService` at every point a credential becomes an identity (`validate_token` for both session tokens and external JWTs, `authenticate_user`, `verify_credentials`), so a blocked user cannot authenticate through any surface — chat, admin panel, file upload, A2A, voice, or discovery. Adding a rule revokes matching users' live sessions immediately rather than waiting for token expiry, and reports how many users matched and how many sessions were cut. A rule matching the requesting administrator is rejected, so an operator cannot lock themselves out of the panel they'd need to undo it. Rules are stored in a new `user_blacklist` table (SQLite/Postgres/MongoDB), managed from the Users tab's "Blocked Identities" panel or `GET`/`POST`/`PUT`/`DELETE /auth/blacklist` gated on `users.manage`. Existing rules are editable inline from a per-row pencil control — correcting a typo'd pattern no longer means deleting and re-creating the rule — and an edit re-revokes sessions for whoever the new pattern matches, since changing a pattern changes who is blocked. Because wildcard matching runs in Python, each worker caches the rule set for `auth.blacklist.cache_ttl` seconds (default 30), which bounds propagation under `performance.workers > 1`; session revocation is immediate regardless. Rule mutations are recorded in the admin audit trail as `auth.blacklist.create`/`update`/`delete` against resource type `blacklist_rule`, capturing the pattern and reason so the ledger shows who was blocked and why.

### Security
- **Require Authenticated User on Inference (`auth.require_authenticated_user`)**: New flag, default `false`, that makes a valid `Authorization: Bearer <user-token>` mandatory on every inference surface (chat, files, voice, A2A) in addition to the API key. This closes a gap in the user blacklist above: previously, API-key-only requests never resolved a user identity, so `validate_token` — where blacklist rules and per-key `allowed_user_ids`/`allowed_emails` are enforced — was never called, and a blocked user could reach inference by simply omitting the bearer token. With the flag on, `Authorization: Bearer` is reserved for the user token (the OpenAI-SDK-compatible fallback that read it as the API key is disabled — the key must go in `X-API-Key`), and the unverified `X-User-ID` header is ignored in favor of the resolved identity. An adapter can override the global setting via `capabilities.requires_authenticated_user` (`true`/`false`), for a mixed deployment of public and locked-down adapters. Bypass lanes that would otherwise skip API-key validation (missing key service, `api_keys.enabled: false`, an empty key resolving via `allow_default`) now fail closed under the flag rather than silently exempting themselves; `/health` remains reachable without credentials. The `/mcp` mount, which bypasses ORBIT's normal auth entirely and cannot honor the flag, is refused at startup with a warning when the flag is on. **Breaking when enabled**: OpenAI SDK clients sending the key as `Authorization: Bearer` need `default_headers={"X-API-Key": "..."}` instead; A2A callers need `X-ORBIT-User-Authorization`; voice WebSocket clients need a handshake `Authorization` header or `?access_token=`. Default is off, so no existing deployment is affected until an operator opts in.
