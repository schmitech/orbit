...
- **Route Configurator Cleanup**: Collapsed the four hand-rolled lazy-init-on-`app.state` service dependencies in `routes_configurator.py` (health, thread, feedback, autocomplete) into a single `_create_lazy_service_dependency` helper, and replaced the five copy-pasted optional-router `try/except` registration blocks in `_include_admin_routes` (metrics, admin panel, file, voice, A2A) with a declarative table and loop. Also removed two redundant per-request `import json` statements in the `create_thread` route handler (the module-level import was already available). No behavior change. Added 25 new tests under `server/tests/test_routes/` covering the refactored paths: `test_lazy_service_dependency.py` (caching, `initialize()` invocation, and that each real factory delegates to the shared helper with its own attribute name), `test_optional_router_registration.py` (all five optional routers reachable when healthy, a failing import or factory for one router doesn't block or crash the others, required routers stay up regardless), and `test_create_thread_route.py` (metadata normalization across dict/JSON-string/`metadata_json`-column forms, plus the existing validation/error paths). Remaining findings from the same review (API key resolution consolidation, thread/feedback authorization-check overlap, the dependency dict-of-closures indirection, and the OpenAI-stream serialize/reparse round trip) are tracked as phased follow-ups in `docs/roadmap/routes-configurator-cleanup.md`.
- **Route Configurator Cleanup (Phase 1)**: Added a shared `resolve_api_key(request, config, allow_bearer_fallback)` helper in `routes/auth_helpers.py` and switched `RouteConfigurator._resolve_api_key` and the inline extraction in `get_api_key` to call it, removing the duplicated "X-API-Key, else Authorization: Bearer" parsing between them. Each call site passes the same bearer-fallback condition it enforced before (`True`, and `not global_strict` respectively), so behavior is unchanged. The header-only call sites in `auth_dependencies.py`, `file_routes.py`, and `discovery_routes.py` were deliberately left unmigrated, since they never accepted bearer fallback and migrating them was assessed as low value relative to the risk. No behavior change. Added `test_resolve_api_key.py` covering the shared helper directly.
- **Route Configurator Cleanup (Phase 2)**: Added `RouteConfigurator._check_session_authorized()`, a shared one-line wrapper around `chat_history_service.authorize_session` used by both the thread and feedback session-ownership checks. Each caller keeps its own availability check and status-code mapping (403 fail-closed for the thread path's legacy-ownership fallback, 503 service-unavailable for feedback) unchanged — only the single duplicated call was extracted. No behavior change.
- **Route Configurator Cleanup (Phase 3)**: `_create_dependencies` now sets each FastAPI dependency as a named instance attribute (`self.get_chat_service`, `self.get_api_key`, etc.) instead of building a `dict[str, Callable]` threaded through every `_configure_*_endpoint` method; those methods now read `self.get_x` directly instead of taking a `dependencies` parameter. Test helpers that previously injected fakes via a raw dependencies dict now set the same attributes on the configurator instance. No behavior change.
...
### Bug Fixes
- **MCP Admin Panel: Pending Header/Env Var Changes**: Existing-server editors now detect and save uncommitted add-row values, with the same key-format validation used during server creation.
...
### CLI & Tools
- **`orbit status` Redesign**: `status` now reports adapter readiness, request/error rates, latency percentiles, circuit-breaker issues, and an overall healthy/degraded/unhealthy verdict. It adds detailed and JSON output, live watch rendering, and clearly reports unavailable monitoring or admin sections.

### Bug Fixes
- **Chunking Safeguards (Phases 3–5)**: Hardened embedding fallback and recovery with model-aware budgets, structured provider-error handling, bounded retries/concurrency, deterministic idempotent ingestion, generation isolation, and safe Chroma retries. Retrieval output is now always budgeted, with truncation and partial-ingestion status disclosed.
- **Retriever and Fault-Tolerance Reliability**: Fixed truncated identifiers and related typos, including a circuit-breaker initialization failure that broke readiness and system health reporting.
- **Request Metrics Accuracy**: Admin, auth, and health routes now use full path templates and are excluded correctly from application metrics; CLI status probes no longer skew traffic statistics.
- **CLI Fixes**: Corrected one-shot CPU reporting and updated usage examples to invoke the platform-specific `bin/` wrapper.
...
### Breaking Changes
- **Audit Chat Call Type**: Renamed text-generation audit values from `inference` to `chat`, including `source=chat`, audit fields, and the usage API filter. Clients using the old values must update.

### Core System Updates
- **Intent Template Retrieval**: Completed production metrics and an Admin Panel Misses view with candidate scores and diagnostic re-tests. Added opt-in confidence-based disambiguation and slot-fill clarification with bounded session state, streaming support, and dedicated telemetry.
- **Audit and Retention Updates**: Updated Audit and Costs terminology to classify text generation as Chat, enabled configurable audit-store clearing on startup, and added independent admin-event retention controls (disabled by default).

### Security
- **Presidio PII Moderation**: Added opt-in Presidio REST-based PII detection with bounded, safe batch processing.
- **Admin Security Hardening**: API-key renames now use validated JSON bodies instead of URL parameters, and login/SSO redirects reject external or protocol-relative destinations.

### Documentation & Configuration
- **ORBIT Capability Matrix**: Added a sourced Markdown and PDF comparison of ORBIT with LiteLLM, Open WebUI, and commercial AI gateways, linked from the README.

### Tools & Utility Scripts
- **Markdown-to-PDF Script**: Added a reusable pandoc and headless Chrome utility for converting Markdown documents to landscape PDFs.

### Bug Fixes & Technical Improvements
- **Skill Routing and Retrieval**: Fixed no-match handling, uploaded-file retrieval, autocomplete scoping, and automatic document-skill routing. Document-generation skills now refuse clearly when requested source material is unavailable instead of inventing filler content.
- **Azure AI Foundry Provider Migration**: Migrated Azure inference to the unified `/openai/v1` endpoint and current token parameter, added named deployment presets, and removed the retired client dependency.
