...
- **API Key Reactivation**: The API Keys detail view's Danger Zone now shows an "Activate Key" action for a deactivated key (mirroring the existing "Deactivate Key" action), backed by a new `POST /admin/api-keys/{id}/activate` endpoint and `ApiKeyService.activate_api_key_by_id()`. Previously a deactivated key had no path back to active in the admin panel.
...
- **Chunking Safeguards (Phase 1b)**: Extended the shared embedding budget to uploaded-file ingestion in `FileVectorRetriever.index_file_chunks()`. Validates and re-splits document chunks via `split_text_to_budget` immediately before embedding submission (`embed_documents_tracked`, `embed_documents`, or `embed_query`), preventing oversized chunks regardless of chunker origin. Sub-chunks are re-indexed to ensure strictly sequential, unique `chunk_index` values with parent metadata (`parent_chunk_id`, `parent_chunk_index`, `split_index`, `split_count`). Introduced `TokenInt` so character and fallback tokenizers explicitly label counts with `estimated=True`, aligned decode-failure estimation in `TokenChunker` and `FixedSizeChunker` to the shared 1:3 character-to-token ratio, and enforced callable `encode` and `decode` protocols on custom tokenizers.
