...
- **MCP Connection Editing**: The MCP admin panel can now edit stdio server `command`, `args`, and `env` settings, and HTTP/SSE server `headers`, with each save applied immediately through the existing scoped reload. Connection fields are transport-specific: HTTP headers are not exposed or accepted for stdio servers because that transport cannot consume them.
