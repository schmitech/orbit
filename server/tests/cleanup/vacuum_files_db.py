...
This script vacuums the main backend SQLite database (orbit.db) to reclaim space
after deletions. VACUUM rebuilds the database file, removing unused space.

Note: This script only works when using SQLite as the backend database.
For MongoDB, use MongoDB's compact command instead.
...
class DatabaseVacuum:
    """Vacuum the main SQLite backend database to reclaim space."""
    ...
    def _get_db_path(self) -> Optional[str]:
        ...
        try:
            ...
            backend_type = backend_config.get('type', 'sqlite')

            if backend_type != 'sqlite':
                logger.error(f"Backend type is '{backend_type}', not 'sqlite'. This script only works with SQLite backend.")
                logger.info("For MongoDB, use MongoDB's compact command instead.")
                return None

            # Get SQLite database path from internal_services.backend.sqlite.database_path
            db_path = backend_config.get('sqlite', {}).get('database_path', 'orbit.db')
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                result = cursor.fetchone()
                stats['chat_history_count'] = result[0] if result else 0
            except sqlite3.OperationalError:
            ...
            try:
                ...
                result = cursor.fetchone()
                stats['user_count'] = result[0] if result else 0
            except sqlite3.OperationalError:
                stats['user_count'] = 0
    ...
    def print_stats(self, stats: Dict[str, Any], label: str = "Current"):
        ...
        print("\n  Record counts:")
        print(f"    - Uploaded files: {stats.get('file_count', 0):,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Vacuum the main SQLite backend database (orbit.db) to reclaim space",
...
Note: This script only works with SQLite backend. For MongoDB, use
MongoDB's compact command instead.
        """
    )
    ...
    vacuum = DatabaseVacuum(dry_run=args.dry_run)
