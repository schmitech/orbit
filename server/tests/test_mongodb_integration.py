...
def test_services_share_mongodb_singleton(mock_config):
    ...
    # Create services that internally create MongoDB instances
    api_key_service = ApiKeyService(mock_config)  # No mongodb_service passed
    auth_service = AuthService(mock_config)       # No mongodb_service passed  
    prompt_service = PromptService(mock_config)   # No mongodb_service passed
    
    # All should have the same MongoDB instance due to singleton
    assert api_key_service.mongodb is auth_service.mongodb
    assert auth_service.mongodb is prompt_service.mongodb
    assert api_key_service.mongodb is prompt_service.mongodb
...
def test_service_factory_and_services_share_mongodb(mock_config):
    ...
    assert api_key_service.mongodb is shared_mongodb
...
def test_mongodb_singleton_with_different_databases():
    ...
    # Should have different MongoDB instances
    assert service1.mongodb is not service2.mongodb
