...
class SQLiteService(DatabaseService):
    ...
    async def create_index(
        ...
        # Callers may pass MongoDB-style "_id" (e.g. to mirror a compound index
        # ending in the document id across all three backends) — map it to the
        # actual storage column name, same as _projection_storage_columns does
        # for find_many.
    ...
    async def delete_messages_beyond_token_budget(
        ...
        try:
            ...
            base_sql = f'''
                SELECT id, timestamp,
                       CASE WHEN "{token_field}" IS NOT NULL THEN "{token_field}"
                            ELSE MAX(1, LENGTH("{content_field}") / ?) END AS effective_tokens
                FROM {collection_name}
                WHERE {where_clause}
            '''
            ranked_sql = f'''
            ...
            '''
            boundary_sql = f'''
                SELECT id, timestamp FROM ({ranked_sql})
                WHERE running_total > ?
            ...
            delete_predicate = '(timestamp < ? OR (timestamp = ? AND id <= ?))'
            delete_params = (b_ts, b_ts, b_id)

            # DELETE ... RETURNING computes and deletes in one atomic statement
            # (single connection, single lock acquisition) — deleted_count and
            # tokens_removed are derived only from the rows this statement
            # actually deleted, never from a separate tally that could race
            # against a concurrent writer touching the same session.
            delete_sql = f'''
                DELETE FROM {collection_name}
                WHERE {where_clause} AND {delete_predicate}
                RETURNING (
                    CASE WHEN "{token_field}" IS NOT NULL THEN "{token_field}"
                         ELSE MAX(1, LENGTH("{content_field}") / ?) END
                ) AS effective_tokens
            '''
