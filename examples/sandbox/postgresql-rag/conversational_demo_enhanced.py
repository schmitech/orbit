#!/usr/bin/env python3
"""
Enhanced Interactive Demo for Domain-Agnostic RAG System
...
class ConversationalDemo:
    """Enhanced demo with domain configuration support"""
    
    def __init__(self, use_existing_templates: bool = True):
        self.rag_system = None
        self.domain = None
        self.template_library = None
        self.query_history = []
        self.use_existing_templates = use_existing_templates
        self.setup_readline()
    ...
    def print_header(self):
        ...
        print("="*80)
        print("This enhanced system uses domain configuration for flexibility!")
        print("\nFeatures:")
    ...
    def load_or_generate_templates(self) -> TemplateLibrary:
        """Load existing templates or generate from domain"""
        if self.use_existing_templates and os.path.exists("query_templates.yaml"):
            print("📚 Loading existing query templates...")
            library = TemplateLibrary(self.domain)
            library.import_from_yaml("query_templates.yaml")
            return library
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Domain-Agnostic RAG System Demo")
    parser.add_argument(
        "--use-existing-templates",
        action="store_true",
        default=True,
        help="Use existing query_templates.yaml instead of generating from domain"
    )
    parser.add_argument(
        "--generate-templates",
        action="store_true",
        help="Generate templates from domain configuration"
    )
    
    args = parser.parse_args()
    
    # If generate-templates is specified, override use-existing-templates
    if args.generate_templates:
        use_existing = False
    else:
        use_existing = args.use_existing_templates
    
    demo = ConversationalDemo(use_existing_templates=use_existing)
    demo.run()
