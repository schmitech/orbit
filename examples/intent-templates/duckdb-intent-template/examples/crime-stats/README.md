...
The fastest way to go from a raw CSV to a loaded DuckDB database is `utils/duckdb/init_dataset.py`. It detects the schema, generates SQL and import config, and registers the dataset — all in one command.

```bash
# 1. Scaffold everything (SQL schema, import_config.yaml, csv_to_duckdb.yaml entry)
python utils/duckdb/init_dataset.py data.csv --name my-dataset --description "My Dataset"

# 2. Review the generated files, then load
python utils/duckdb/generate_duckdbs.py my-dataset --clean

# Or do both in one shot:
python utils/duckdb/init_dataset.py data.csv --name my-dataset --load
```

Preview what would be generated without writing files:
```bash
python utils/duckdb/init_dataset.py data.csv --name my-dataset --dry-run
```
...
```bash
python utils/duckdb/generate_duckdbs.py all --clean
```
...
The steps below show the individual scripts that `utils/duckdb/init_dataset.py` automates. Use these when you need finer control.

### 1. Generate the SQL schema and import config

```bash
# Generate schema.sql + import_config.yaml with real column mappings
python utils/duckdb/generate_schema.py data.csv --config

# Schema only (no import config)
python utils/duckdb/generate_schema.py data.csv --output data_schema.sql --table my_table

# Preview without writing
python utils/duckdb/generate_schema.py data.csv --config --dry-run
```
...
To regenerate just the import config against an existing (possibly hand-edited) SQL schema, use `utils/duckdb/generate_import_config.py`:

```bash
python utils/duckdb/generate_import_config.py data.csv --schema data_schema.sql
python utils/duckdb/generate_import_config.py data.csv --schema data_schema.sql --dry-run
```
...
`utils/duckdb/generate_schema.py`, `utils/duckdb/generate_import_config.py`, and `utils/duckdb/csv_to_duckdb.py` expose DuckDB CSV reader knobs for messy files:
...
python utils/duckdb/csv_to_duckdb.py huge.csv --auto-split --schema huge.sql --output huge.duckdb

# Custom chunk size (split into 25 MB chunks)
python utils/duckdb/csv_to_duckdb.py huge.csv --split-size 25 --schema huge.sql --output huge.duckdb
```
...
1.  **`utils/duckdb/init_dataset.py`**: One-command scaffold — detects schema, generates SQL + import config, registers the dataset in `csv_to_duckdb.yaml`. Optionally loads the database with `--load`.
2.  **`utils/duckdb/generate_schema.py`**: Auto-detects column types from the CSV and writes a SQL schema file. With `--config`, also produces an `import_config.yaml` with real column mappings.
3.  **`utils/duckdb/generate_import_config.py`**: Matches schema columns to CSV headers and writes an `import_config.yaml`.
4.  **`utils/duckdb/generate_duckdbs.py`**: Reads `csv_to_duckdb.yaml`, identifies the target dataset(s), and calls `utils/duckdb/csv_to_duckdb.py`.
5.  **`utils/duckdb/csv_to_duckdb.py`**:
