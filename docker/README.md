...
./publish.sh --build --tag v2.15.2

# Build a targeted OpenAI image from release v2.15.2 with custom config
./publish.sh --build --tag 2.15.2-openai --config-dir ../deploy/openai

# Build and publish to Docker Hub
./publish.sh --publish

# Build and publish the default Ollama/basic image with version tags
./publish.sh --publish --tag v2.15.2

# Build and publish a targeted OpenAI image
./publish.sh --publish --tag 2.15.2-openai --config-dir ../deploy/openai
```
...
./publish.sh --build --source local-tarball --tag v2.15.2
...
```bash
./publish.sh --build --tag 2.15.2-openai --config-dir ../deploy/openai
./publish.sh --build --tag 2.15.2-gemini --config-dir ../deploy/gemini
./publish.sh --build --tag 2.15.2-ollama --config-dir ../deploy/ollama
```

For release builds, tags like `2.15.2-openai` automatically resolve the source
tarball from release `v2.15.2`. Use `--release-tag v2.15.2` when you want to be
explicit.

Custom config builds require `--tag` and do not update the generic `latest` or
`basic` tags. They create the targeted tag, such as
`schmitech/orbit:2.15.2-openai`, plus the compatibility tag
`schmitech/orbit:basic-2.15.2-openai`.
...
```bash
docker pull schmitech/orbit:2.15.2-openai
docker run -d --name orbit-2.15.2-openai \
  --env-file ./orbit.env \
  -p 3000:3000 \
  schmitech/orbit:2.15.2-openai
```
...
./publish.sh --build --tag v2.15.2  # Build from a specific release tarball
./publish.sh --build --tag 2.15.2-openai --config-dir ../deploy/openai
./publish.sh --publish            # Build and push to Docker Hub
./publish.sh --publish --tag v2.15.2  # Build, push, and tag default image
./publish.sh --publish --tag 2.15.2-openai --config-dir ../deploy/openai
