...
# Build a targeted OpenAI image from release v2.8.0 with custom config
...
./publish.sh --build --source local-tarball --tag v2.8.0
...
```bash
./publish.sh --build --tag 2.8.0-openai --config-dir ../deploy/openai
./publish.sh --build --tag 2.8.0-gemini --config-dir ../deploy/gemini
./publish.sh --build --tag 2.8.0-ollama --config-dir ../deploy/ollama
```

For release builds, tags like `2.8.0-openai` automatically resolve the source
tarball from release `v2.8.0`. Use `--release-tag v2.8.0` when you want to be
explicit.

Custom config builds require `--tag` and do not update the generic `latest` or
`basic` tags. They create the targeted tag, such as
`schmitech/orbit:2.8.0-openai`, plus the compatibility tag
`schmitech/orbit:basic-2.8.0-openai`.
...
```bash
docker pull schmitech/orbit:2.8.0-openai
...
./publish.sh --build --tag v2.8.0  # Build from a specific release tarball
./publish.sh --build --tag 2.8.0-openai --config-dir ../deploy/openai
./publish.sh --publish            # Build and push to Docker Hub
./publish.sh --publish --tag v2.8.0  # Build, push, and tag default image
./publish.sh --publish --tag 2.8.0-openai --config-dir ../deploy/openai
