...
# Set Python path to include project root and server directory
...
# Always use the correct module path for the project structure
...
if [ "$RELOAD" = "true" ]; then
    ...
    if [ "$WORKERS" -gt "1" ]; then
        ...
        if [ -n "$CONFIG_PATH" ]; then
            PYTHON_CMD="cd $PROJECT_ROOT/server && python main.py --config \"$PROJECT_ROOT/$CONFIG_PATH\""
        else
            PYTHON_CMD="cd $PROJECT_ROOT/server && python main.py"
        fi
