...
interface YamlConfig {
  ...
  adapters?: Array<{ id: string; name: string; apiUrl?: string; description?: string; notes?: string; model?: string; inputPlaceholder?: string; icon?: string }>;
}

const GENERATED_SITEMAP_MARKER = '<!-- Generated by orbitchat-config -->';
const GENERATED_ROBOTS_MARKER = '# Generated by orbitchat-config';

function slugifyAdapterName(name: string): string {
  return name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
function normalizeConcreteOrigin(value: string): string | null {
...
function buildSitemapXml(config: YamlConfig | null, adapterMap: Record<string, AdapterConfig> | null): string | null {
  if (!config?.seo?.includeSitemap) {
    return null;
  }

  const canonicalOrigin = normalizeConcreteOrigin(config.seo.siteUrl?.trim() || '');
  if (!canonicalOrigin) {
    console.warn('[orbitchat-config] seo.includeSitemap is enabled but seo.siteUrl is empty; sitemap.xml will not be generated.');
    return null;
  }

  const alternateOrigins = Array.isArray(config.seo.alternateSiteUrls)
    ? config.seo.alternateSiteUrls
        .map((value) => normalizeConcreteOrigin(value))
  ...
  if (wildcardEntries.length > 0) {
    console.warn('[orbitchat-config] seo.hostPatterns supports wildcard hosts at runtime, but wildcard hosts are not added to sitemap.xml.');
  }
  ...
  if (mode === 'single') {
    ...
    Object.keys(adapterMap || {}).forEach((adapterId) => {
  ...
  if (urls.size === 0) {
    console.warn('[orbitchat-config] No sitemap URLs were generated from the configured adapters.');
    return null;
  }
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n${GENERATED_SITEMAP_MARKER}\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urlEntries}\n</urlset>\n`;
}

function buildRobotsTxt(config: YamlConfig | null): string | null {
  const canonicalOrigin = normalizeConcreteOrigin(config?.seo?.siteUrl?.trim() || '');
  if (!canonicalOrigin) {
    return null;
  }

  return `${GENERATED_ROBOTS_MARKER}
User-agent: *
Allow: /
...
function syncGeneratedSitemap(publicDir: string, sitemapXml: string | null): void {
  const sitemapPath = path.join(publicDir, 'sitemap.xml');
  const existing = fs.existsSync(sitemapPath) ? fs.readFileSync(sitemapPath, 'utf8') : null;

  if (!sitemapXml) {
    if (existing?.includes(GENERATED_SITEMAP_MARKER)) {
  ...
  if (existing === sitemapXml) {
    return;
  }
  fs.writeFileSync(sitemapPath, sitemapXml, 'utf8');
}

function syncGeneratedRobots(publicDir: string, robotsTxt: string | null): void {
  const robotsPath = path.join(publicDir, 'robots.txt');
  const existing = fs.existsSync(robotsPath) ? fs.readFileSync(robotsPath, 'utf8') : null;

  if (!robotsTxt) {
    if (existing?.includes(GENERATED_ROBOTS_MARKER)) {
  ...
  if (envKeysRaw) {
    try {
      ...
      for (const [id, value] of Object.entries(keys)) {
        ...
        const icon = isObjectValue ? (value as { icon?: unknown }).icon : undefined;
        ...
        if (typeof icon === 'string') adapters[id].icon = icon;
      }
    } catch { /* ignore */ }
  }
...
export function orbitchatConfigPlugin(): Plugin {
  ...
  return {
    ...
    config(_, { mode }) {
      ...
      syncGeneratedSitemap(publicDir, buildSitemapXml(yamlConfig, adaptersCache));
      syncGeneratedRobots(publicDir, buildRobotsTxt(yamlConfig));
