...
| **Identity Blacklisting** | Not applicable — virtual keys are allow-listed, no deny-by-identity-pattern mechanism | Pattern-based blacklist (wildcards on email/user_id/username) blocks even external Entra/Auth0 users who have never logged in, revokes matching live sessions immediately, enforced at every point a credential becomes an identity |
| **Mandatory User Auth on Inference** | Not applicable — virtual keys themselves are the identity | Optional `auth.require_authenticated_user` flag (global or per-adapter) forces a verified user token alongside the API key on every inference surface, so key-only requests can't bypass user-level blocks |
...
- **Identity-pattern blacklisting**: ORBIT can deny authentication by shell-style wildcard pattern against email, user_id, or username — blocking even external Entra/Auth0 users who have never signed in (JIT provisioning is refused for a blacklisted identity) — and revokes matching live sessions immediately rather than waiting for token expiry. LiteLLM has no equivalent deny-by-identity-pattern mechanism; virtual keys are an allow-list construct. ORBIT closes the gap where API-key-only requests bypass user-level checks with an optional `auth.require_authenticated_user` flag that makes a verified user token mandatory on every inference surface.
