...
| **Identity Blacklisting** | User deactivation requires an existing account row | Pattern-based blacklist (wildcards on email/user_id/username) blocks even external Entra/Auth0 users who have never logged in, revokes matching live sessions immediately, and is enforced at every point a credential becomes an identity |
| **Mandatory User Auth on Inference** | Not applicable — Open WebUI's own login gates its own UI | Optional `auth.require_authenticated_user` flag (global or per-adapter) forces a verified user token on every inference surface (chat, files, voice, A2A) in addition to the API key, closing the gap where key-only requests bypass user-level blocks |
...
| **File Encryption at Rest** | Not available — application-level file encryption is an open feature request; only DB-level SQLCipher is supported today | Native AES-256-GCM, opt-in per adapter — covers uploaded file bytes, storage metadata, and indexed vector-store chunk content, on any storage backend |
...
ORBIT also goes further than allow-listing with a deny-list: administrators can block identities by shell-style wildcard pattern (`*@spam-domain.com`, `entra:abc*`) against email, user_id, or username. Unlike deactivating a user — which needs an existing row in the `users` table — a blacklist rule also stops external Entra/Auth0 users from ever being JIT-provisioned in the first place, and it revokes matching users' live sessions immediately rather than waiting for token expiry. Enforcement is centralized in `AuthService` at every point a credential becomes an identity, so a blocked user cannot get through chat, admin, file upload, A2A, or voice. Because API-key-only requests never resolved a user identity by default (leaving blacklist and per-key allowlist checks unreachable), the optional `auth.require_authenticated_user` flag closes that gap by making a verified user token mandatory on every inference surface, with a per-adapter override for mixed public/locked-down deployments.
