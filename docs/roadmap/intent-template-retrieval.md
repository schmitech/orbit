...
## Phase 5 — Graceful degradation (behind `clarification_enabled: false`) — ✅ DONE
...
**Shipped as:** all in `retrievers/base/intent_sql_base.py`, gated by `clarification_enabled` (default `false`) plus `clarification_high_threshold` (0.65), `clarification_ambiguity_gap` (0.05), `clarification_max_rounds` (2), `clarification_ttl_seconds` (300), and `no_match_message` — read from `adapter_config` alongside the other Phase 2B/3B knobs (flat keys, not the plan's nested `intent.clarification.*`, for consistency with `query_guard_enabled`/`confidence_threshold`). The per-template execute/format block was extracted into `_format_execution_response()` so both the normal matching loop and the clarification-resume path share it instead of duplicating ~100 lines of result formatting. New pending-clarification state lives in `server/services/intent_clarification_state.py` — an in-memory, TTL-bounded `(adapter, session_id) -> payload` store (mirrors `template_misses.py`'s process-local/best-effort posture) that `_build_disambiguation_response()`/`_build_slot_fill_clarification()` write to and `_resume_pending_clarification()` reads on the next turn for that session. `IntentClarificationStep` (`inference/pipeline/steps/intent_clarification.py`) is wired into `pipeline.py` right after `ContextRetrievalStep`; on `metadata.intent_action == "clarify"` it sets `context.response` and calls `context.set_error(..., block=True)`, exactly the `FetchStep` pattern, so the LLM is skipped and the question streams back as the answer. `record_intent_telemetry` (Phase 4A) now also records `clarify_disambiguate`/`clarify_slot_fill` outcomes via the existing `orbit_intent_template_matches_total` metric, so Phase 4's dashboard shows clarification rate for free.

Deviated from plan in three respects, all scope cuts:
1. **Resume requires `capabilities.supports_session_tracking: true` on the adapter.** That's how `session_id` reaches `get_relevant_context()` today (`adapters/capabilities.py::build_retriever_kwargs`); without it a clarify question is still asked, but the next turn can't be resumed against the pinned template and just re-matches from scratch. Worth calling out in adapter setup docs rather than silently assumed.
2. **No pinned-template hint threaded through `_find_best_templates()`.** Resume re-fetches the pinned template directly via `domain_adapter.get_template_by_id()` instead — simpler, and avoids changing `_find_best_templates()`'s signature/behavior for the (much more common) non-resume path.
3. **HTTP and Composite bases unchanged.** The plan's line references (`intent_sql_base.py:733` etc.) already scoped this to the SQL base; mirroring it into `intent_http_base.py`/`intent_composite_base.py` is a natural follow-up once the band thresholds are validated against real traffic, consistent with how Phase 3B's query guard landed SQL-first.

Verified: full `test_retrievers/` suite green (433 passed, 2 skipped — 24 new in `test_intent_clarification.py` covering band classification, disambiguation/slot-fill response building, the pending store's TTL/round bookkeeping, and resume with a changed/removed template hash); HR SQL eval baseline unchanged (148/149, 100% recall@3/@5) confirming `clarification_enabled: false` (the default) leaves matching byte-identical to Phase 4, since every new branch is gated on that flag.
...
- **5** — ✅ with `clarification_enabled: true`: (a) an ambiguous query returns a `clarify/disambiguate` response and `IntentClarificationStep` blocks the pipeline before LLM inference (unit-verified: band classification, response shape, and the pipeline step's `should_execute`/block behavior); (b) a top-template match missing a required parameter returns `clarify/slot_fill`, and resuming with the answer merges it in and executes (or re-asks for any still-missing param, bumping the round, up to `clarification_max_rounds`); (c) the pending store's TTL and round cap both terminate correctly (unit-tested directly); (d) with the flag **off** (the default), the HR SQL eval baseline is unchanged (148/149, 100% recall@3/@5) — every new branch is gated on `clarification_enabled`. **Gap:** not exercised end-to-end through a live pipeline run (streaming HTTP request → clarify → follow-up turn) — verified at the retriever/pipeline-step unit level instead; flagged rather than assumed.
