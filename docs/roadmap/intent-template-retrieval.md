...
**Status:** 4A and 4B shipped, both scoped down from the original plan (see deviations below).
...
**Shipped as:** `MetricsService` gains `orbit_intent_template_matches_total{adapter,template_id,outcome}`, `orbit_intent_confidence{adapter}` (Histogram), `orbit_intent_rows_returned{adapter,template_id}` (Histogram), `orbit_intent_row_cap_applied_total{adapter}`, and `orbit_intent_guard_rejections_total{adapter,reason}` — plus `set_metrics_service_instance()`/`get_metrics_service_instance()` module-level accessors (mirroring the existing `mcp_client_service.py` singleton pattern), since retrievers have no access to `app.state`. `service_factory.py`'s `_initialize_metrics_service` calls `set_metrics_service_instance()` right after constructing the service. Both `intent_sql_base.py` and `intent_http_base.py` now wrap their (renamed) `_get_relevant_context_impl` in a thin `get_relevant_context` that calls a new `_record_intent_telemetry()` — a single instrumentation point reading the outcome straight off the metadata dict already returned by every branch, rather than editing every return statement individually. `outcome` values: `executed | no_match | param_validation_failed | datasource_unavailable | error`. The query guard's cap-clamp/injection is detected in `intent_sql_base.py::_execute_template` by comparing the SQL text before/after `enforce_row_cap`; guard rejections are recorded where `QueryGuardError` is already caught.

Deviated from plan in two respects, both scope cuts:
1. **No `TemplateTrace`/sample-rate/audit-sink persistence.** The plan's trace-sampling-into-audit-DB piece (`diagnostics.sample_rate`, `context.metadata["_trace"]`, `ContextRetrievalStep` stripping it before the LLM prompt) was not built. The existing on-demand diagnostics path (Phase 1B, `/admin/adapters/{name}/test-query`) already gives full per-request traces on request; wiring a sampled, persisted version into the live request path is real additional work (new metadata channel through the pipeline, a persistence schema, a sampling knob) deferred as a distinct follow-up rather than bundled in here.
2. **No `orbit_intent_stage_seconds{adapter,stage}` per-stage timing histogram.** `template_diagnostics.py` already times each stage for on-demand test-query calls, but wiring that same instrumentation into the production `get_relevant_context` path (not just the diagnostics path) needs the stage-timing calls threaded through `_find_best_templates`/`_extract_parameters`/`_execute_template` themselves — cut for the same reason as (1).

Verified: `curl localhost:<port>/metrics | grep orbit_intent` after driving a few test-query calls shows `orbit_intent_template_matches_total` and `orbit_intent_confidence` with real label values; full `test_retrievers/` suite green (394 passed, 2 skipped) confirming the `get_relevant_context` rename/wrap didn't change any matching/execution behavior.

**Post-ship fixes from review, round 1:**
1. **[P2] `CompositeIntentRetriever`, `IntentFirecrawlRetriever`, and `IntentAgentRetriever` override `get_relevant_context` entirely, so they never ran the base classes' instrumentation.** Their no-match/below-threshold requests recorded neither metrics nor misses, despite being test-query-capable adapters. Fixed by extracting the duplicated `_record_intent_telemetry` method out of `intent_sql_base.py`/`intent_http_base.py` into a single free function, `record_intent_telemetry(retriever, query, result)`, in `intent_domain_components.py` — a plain function rather than a mixin method specifically so `CompositeIntentRetriever` (which doesn't share a base class with the SQL/HTTP retrievers) can call it too. All five retrievers now follow the same rename-and-wrap shape: the previous `get_relevant_context` body becomes `_get_relevant_context_impl`, and a new thin `get_relevant_context` calls it then `record_intent_telemetry(self, query, result)`. For `IntentAgentRetriever` specifically, its `_template_matching_flow`'s two dead-end paths (`if not matching_templates: return []` and the end-of-loop `return []`) were also converted to metadata-carrying dicts (`error: "no_matching_template"` / `"below_threshold"` / `"parameter_extraction_failed"`, mirroring the SQL/HTTP shape) — the wrapper alone can't record anything useful from a bare `[]`. `CompositeIntentRetriever`'s own telemetry is recorded under its own adapter name, in addition to (not instead of) whatever the routed-to child adapter records under its own name when it delegates via `source_adapter.get_relevant_context(...)` — deliberate, since the two answer different questions (composite-level vs. per-child-adapter).
2. **[P2] The bound-LIMIT clamp path (`LIMIT :limit`) never incremented `orbit_intent_row_cap_applied_total`.** `enforce_row_cap` only clamps a *literal* LIMIT in the SQL text; a bound placeholder is left untouched there and clamped separately by `clamp_bound_limit_parameter`, which previously returned `None` — so the metric only fired for the literal-clamp path enforce_row_cap.py handles, silently underreporting the placeholder-based case that comment in `intent_sql_base.py` explicitly calls out as needing separate handling. Fixed by giving `clamp_bound_limit_parameter` a `bool` return (`True` iff it actually clamped a value) and recording `record_intent_row_cap_applied()` at its call site in `intent_sql_base.py::_execute_template` when it returns `True`.
3. **[P2] The all-below-threshold case was recorded as `param_validation_failed`, not `below_threshold`.** `intent_sql_base.py`, `intent_http_base.py`, and `intent_agent_retriever.py`'s `_template_matching_flow` all fell through to the same generic "couldn't extract the required information" return whether no candidate ever cleared `confidence_threshold` or extraction was attempted and failed — misleading the Misses panel and the outcome metric for pure low-confidence-matching regressions (no parameter validation ever ran). Fixed by tracking whether any candidate cleared the threshold (`any_above_threshold`; HTTP already had an equivalent list, `template_attempts`, which only gains an entry post-threshold) and returning a distinct `error: "below_threshold"` metadata dict when it's empty, mapped to its own `below_threshold` outcome (already the vocabulary `CompositeIntentRetriever` used for its own equivalent case).
4. **[P2] Firecrawl's own override had the same all-below-threshold bug, missed in fix (3) above.** `IntentFirecrawlRetriever.get_relevant_context` has its own inline matching loop (predating the shared bases, for chunking support) rather than reusing `intent_http_base.py`'s or `intent_agent_retriever.py`'s — it wasn't touched when (3) was fixed, so an all-below-threshold Firecrawl query still fell through to a generic `"all_templates_failed"` (mapped to the catch-all `error` outcome, and never recorded as a miss at all, since `record_intent_telemetry` only stores a miss for `no_match`/`below_threshold`/`param_validation_failed`). Fixed the same way as (3): `any_above_threshold` tracked through the loop, a distinct `error: "below_threshold"` + `candidates` return when nothing cleared threshold, and `candidates` added to the `all_templates_failed` return too for consistency.

Verified: `test_retrievers/` suite green (394 passed, 2 skipped) after all four fixes; targeted inline reproductions confirmed `clamp_bound_limit_parameter` returns `True`/`False` correctly, `record_intent_telemetry` records a `below_threshold` miss (not `param_validation_failed`) when given that metadata shape, and the composite/firecrawl/agent overrides now call it.
...
**Shipped as:** `server/services/template_misses.py` — a bounded (500-entry), process-local, in-memory `deque`-backed store with `record_miss()`/`list_misses()`/`record_feedback()`/`list_feedback()`. `_record_intent_telemetry()` (added in 4A, shared by both bases) calls `record_miss()` whenever the outcome is `no_match` or `param_validation_failed`, recording the query text, adapter, reason, top-N candidates (`template_id`+`similarity` where available), the resolved confidence threshold, and a timestamp. Two new admin endpoints in `routes/admin/adapters.py`: `GET /admin/adapters/{name}/misses` (lists misses for one adapter, most recent first) and `POST /admin/adapters/{name}/feedback` (new `TemplateFeedbackRequest` model in `models/schema.py`: `verdict`, `request_id`, `template_id`, `expected_template_id`) — both delegate straight to the store. The admin UI (`adapters.js`) gets a "Misses" `<details>` panel next to Test Query, gated on the same `supports_test_query` capability flag, listing each miss with a "Test in diagnostics" button that opens the Test Query panel prefilled with that exact query and runs it.

Deviated from plan in three respects, all scope cuts:
1. **In-memory, not a durable store.** The plan said "record instead to a `template_misses` store" without mandating persistence; an in-memory bounded deque was chosen over the audit DB to avoid a schema migration and keep this additive — misses don't survive a restart. Promoting to a durable store is flagged in the module's own docstring as future work if the window proves too small.
2. **Feedback is recorded but not yet wired back into the Phase 1A eval corpus.** `POST .../feedback` accepts and stores `expected_template_id`, but nothing yet reads `list_feedback()` to append to `corpora/intent-sql-sqlite-hr.yaml` — that auto-grow step (the plan's "~50 lines for the flywheel") is still manual: a human would read the stored feedback and hand-add corpus entries.
3. **No feedback UI in the admin panel**, only the backend endpoint — the panel's "Test in diagnostics" button covers the review half of the loop; a UI to submit a verdict/expected-template-id was cut as a smaller, separable follow-up.

Verified: unit-level via the full `test_retrievers/` suite (unaffected — this phase touches routes/services, not retrievers' matching logic) plus manual exercise of the new endpoints (`GET .../misses` after a deliberately unmatched query returns it in the list; `POST .../feedback` returns `{"status": "recorded"}` and the entry is retrievable via a direct `list_feedback()` call).
...
- **4A** — ✅ (scoped) `record_intent_outcome`/`record_intent_rows_returned`/`record_intent_row_cap_applied`/`record_intent_guard_rejection` verified directly against a real `MetricsService` instance (module-level singleton wired via `set_metrics_service_instance`/`get_metrics_service_instance`); `test_retrievers/` suite green (394 passed, 2 skipped) confirming the `get_relevant_context` rename/wrap across all five intent retrievers (SQL, HTTP, Composite, Firecrawl, Agent) changed no matching/execution behavior; HR SQL eval regression re-run to baseline (148/149 top-1, 100% recall@3/@5) confirming no accuracy impact. **Not built** (see 4A's own deviation notes): the `_trace`/sample-rate/audit-persistence piece, so there's nothing to assert stays out of the LLM prompt yet — that check applies once the deferred trace-persistence follow-up ships.
- **4B** — ✅ `record_miss`/`list_misses`/`record_feedback`/`list_feedback` verified directly against `services/template_misses.py`; confirmed a `below_threshold`/`no_match`/`param_validation_failed` outcome from any of the five intent retrievers lands in the store with query, adapter, candidates, and threshold, and is listable via `GET /admin/adapters/{name}/misses`; `POST /admin/adapters/{name}/feedback` accepts `expected_template_id` and the entry is retrievable via `list_feedback()`. Admin UI: `adapters.js`'s new Misses panel passes a Node `--check` syntax pass; not exercised in a live browser (see 4B's own deviation notes — feedback submission has no UI yet, only the "Test in diagnostics" jump). Full non-intent server suite (`tests/` minus `intent_eval`/`vllm`) shows 40 pre-existing failures, all confirmed unrelated to Phase 4 (live-server-dependent admin/CLI integration tests, network-dependent inference/embedding tests, and one pre-existing `api_key_service.validate_api_key` signature mismatch against a test's stub) — none touch a file this phase changed.
- **Post-ship fixes from review, round 1** — ✅ verified via the same `test_retrievers/` suite (394 passed, 2 skipped) plus targeted inline reproductions of `clamp_bound_limit_parameter`'s new `bool` return and `record_intent_telemetry`'s `below_threshold` vs. `param_validation_failed` distinction; HR SQL eval baseline unchanged (148/149, 100% recall@3/@5).
