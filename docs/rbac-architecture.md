...
| `operator` | `config.manage`, `adapters.manage`, `apikeys.manage`, `prompts.manage`, `system.manage`, `metrics.read`, `logs.read` | Runs day-to-day system operations (config, adapters, API keys, prompts, restart/shutdown) and can read server logs to troubleshoot them; **cannot** read user conversations/feedback or the audit trail — that visibility belongs to `analyst`/`auditor` |
| `auditor` | `logs.read`, `audit.read`, `metrics.read` | Read-only diagnostics and the audit trail, for compliance/observability roles |
...
This decomposition is what makes it possible, for example, to let an `operator` fully run the system — configuration, adapters, API keys, prompts, restart/shutdown, and reading the resulting server logs — without ever being able to read another user's conversation or the audit trail. That's the sensitivity concern that motivated this design: some admin-panel users should not have permission to read or view conversations, without resorting to an all-or-nothing admin flag. Raw server logs are operational output of the system `operator` already runs, so `operator` holds `logs.read` too; the audit trail (a record of *who did what* across the admin panel) is a distinct, more sensitive concern scoped to `auditor` alone — someone who restarts the server or edits config doesn't automatically need to see a history of every admin action ever taken.
...
A tab can also bundle sub-sections gated by a *different* permission than the tab itself. The Ops tab is visible on `system.manage` alone (it holds restart/shutdown), but its server-log viewer additionally requires `logs.read` — `renderOps()` checks this client-side and skips building the log viewer (and its network calls) entirely rather than rendering a panel that would just 401. `operator` holds both permissions today and sees the viewer; the gate still matters for any future/custom role holding `system.manage` without `logs.read`.
