...
| NIST Control | Documented Capability | Potential Control Contribution | Deployment Evidence & Responsibilities Required |
| :--- | :--- | :--- | :--- |
| **IA-2 Identification & Authentication** | **OIDC/SSO Integration, Native TOTP 2FA & Keyring**: Supports Microsoft Entra ID (Azure AD) and Auth0 OIDC SSO protocols. Password authentication uses PBKDF2-SHA256 hashing. OS Keyring integration is available for credential storage. Local password accounts can additionally enroll native TOTP-based two-factor authentication (`auth.two_factor.enabled`), with `required_for_roles` (default `["admin"]`) mandating enrollment for administrative accounts before they can obtain a full session. | Can participate in MFA enforced by the configured identity provider (federated accounts), and independently enforces a second authentication factor for local accounts, including a mandatory-MFA control for administrative roles. | Integrators must enforce MFA at the OIDC Identity Provider level for federated accounts, and configure `auth.two_factor` (`enabled`, `required_for_roles`, `rate_limit`) plus the `ORBIT_MFA_ENCRYPTION_KEY` secret for local accounts. Evidence: [Two-Factor Authentication implementation plan](https://github.com/schmitech/orbit/blob/main/docs/roadmap/complete/authentication/phase-7-auth-2fa.md) and its verification test suite (`server/tests/test_auth/test_mfa.py`, 13 tests covering enrollment confirmation gating, blocked login without a valid TOTP/recovery code, single-use recovery-code consumption under concurrency, and required-role enrollment/self-disable enforcement). Reference: [Authentication Guide](https://github.com/schmitech/orbit/blob/main/docs/authentication.md). |
| **IA-5 Authenticator Management** | **Local Password Policy & API Key Lifecycle Management**: Local account creation, password changes, resets, and default-admin provisioning enforce configurable length, character-class, and common-password requirements. API keys are securely generated, stored and managed in the API-key store (`api_keys`), and masked in audit ledgers (`orbit_...`). Keys can be deactivated instantly (`active: 0`), bounded by request quotas, or restricted by user/email allowlists. New keys enforce a default 90-day lifetime (`expires_at`, configurable `api_keys.default_lifetime_days`) capped at a maximum (`api_keys.max_lifetime_days`, default 365 days); expired keys are rejected on every API-key authentication path before adapter resolution, allowlists, and quotas. Explicit non-expiring exceptions require admin permission and a recorded justification. Administrators can renew a key's expiration or grant an exception through an audited admin operation; pre-existing keys are migrated to a finite expiration automatically and idempotently on service startup. | Supports password policy enforcement, key management, masking, rapid deactivation, and enforceable finite-lifetime credential management for API keys. | See [Password Complexity implementation plan](https://github.com/schmitech/orbit/blob/main/docs/roadmap/complete/authentication/phase-2-auth-password-complexity.md) and [API Key Expiration implementation plan](https://github.com/schmitech/orbit/blob/main/docs/roadmap/complete/authentication/phase-8-api-key-expiration.md) for the implemented policies and verification coverage. This completes the API-key portion of the IA-5 authenticator-management control contribution: key expiration enforcement contributes to but does not alone satisfy the complete NIST IA-5 requirement without deployment evidence of key issuance/renewal procedures, monitoring of expiring-soon keys, and periodic non-expiring-exception review. References: [API Keys Guide](https://github.com/schmitech/orbit/blob/main/docs/api-keys.md), [SQLite Schema](https://github.com/schmitech/orbit/blob/main/docs/sqlite-schema.md). |
